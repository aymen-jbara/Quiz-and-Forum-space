...
public class CommentPubServiceImpl implements ICommentPubService {
    ...
    public void AddCommentPubToUser(CommentPub e, Long idUser, Long idPub) {
        ...
        e.setPublication(p);
        e.setUser(a);
      if(  BlockCommentsWithInsultingWords(e)==false){
          int nbr=CountComment(idPub);
         p.setNbrComment(nbr+1);
      }
    }

   @Override
   public boolean BlockCommentsWithInsultingWords(CommentPub aa) {
    ...
    public int CountComment(Long idPub) {
        return commentPubRepository.CountComment(idPub);
    }

    @Override
    public List<CommentPub> getAllCommentByPub(Long idPub) {
        return commentPubRepository.getAllCommentByPub(idPub);
    }
}
