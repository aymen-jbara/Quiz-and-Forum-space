...
public interface ConseilsRepo extends JpaRepository<ConseilsUser,Long> {
    ...
    @Query("select a.typeAnswer from QVT qvt join qvt.questions q join q.answers a join a.users u where u.id=:idU ")
    List<TypeAnswer> ListeAnswersQuizzParUser1( @Param("idU") Long idUser);
